...
if file $1 | grep -E ".*:.*c.*program.*text|.*:.*C.*SOURCE.*" > /dev/null
then
	cfile=$1
	testcases=$2
elif file $2 | grep -E ".*:.*c.*program.*text|.*:.*C.*SOURCE.*" > /dev/null
then
	cfile=$2
	testcases=$1
else 
...
# Grep the BIAS of branch 1 (averaged over all test cases)
grep -E -B 1 "^[ ]*branch[ ]*0[ ]*taken[ ]*[0-9]*%" myfile.c.gcov | awk '{if(NR%3 == 1) print $2; if(NR%3 == 2) print $4;}' | awk 'NR%2{printf $0;next;}1'| tr -d "%" | tr ":" "," > branch1_bias.csv
